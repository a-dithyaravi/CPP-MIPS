...
struct Particle {
    ...
    std::array<double, 2> position_unwrapped;
...
public:
    ...
    System(int N, double L) : _numparticles(N), L(L) {
        _particles.resize(N);
    ...
    const std::vector<double>& thetas) 
    {
        ...
        for (int i = 0; i < _numparticles; ++i) {
            ...
            _particles[i].velocity[0] = v0 * _particles[i].n[0];
            _particles[i].velocity[1] = v0 * _particles[i].n[1];
        }
    }
...
public:
    ...
    void step_euler(){
        ...
        for(auto& p : particles){
            ...
            p.velocity[0] = v0 * p.n[0];
            p.velocity[1] = v0 * p.n[1];

            p.position_unwrapped[0] += p.velocity[0] * dt;
            p.position_unwrapped[1] += p.velocity[1] * dt;
